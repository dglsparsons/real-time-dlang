...
algorithm for synchronising these systems \cite{37958}. However, the crucial aspect is the
...
\par\bigskip\noindent
The various methods of implementing ATC 
have been well studied, with implementations existing in both Java
...
necessarily provide immediate cancellation. Brosgol \cite{Brosgol:2002:ATC} 
details that there are two alternative implementations for ATC. 
...
\par\bigskip\noindent
It is therefore possible to determine that ATC
is a fundamental notion for a real-time system. The ability
...
requiring the ability to safely defer throwing of an exception. 
...
Bruno \cite{periodic-java-thread} introduces the concept of a periodic task
...
be executed \cite{ravenscar-java}.  Similarly, Burns and Wellings \cite{burns1998concurrency}
demonstrate how a periodic task can be manually configured in
Ada through the use of the \texttt{delay until} statement.
...
protocols, and finally a method of asynchronous transfer of control. 
