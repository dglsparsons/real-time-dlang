...
desired functionality, the project may then be packaged as an external library. 
...
concurrent control of system components, along with 
interprocess communication. However, as previously shown, the \texttt{Thread}
...
specifically targeted at real-time POSIX compliant operating systems. 
\par\bigskip\noindent
It is worth noting that the underlying operating system may require patching in order 
to be fully viable for a real-time system: it may not be fully
...
concurrency through operating system \texttt{Threads}. These contain the
...
However, no support exists within D's standard libraries to set this
scheduler, therefore, this support has to be supplied. 
...
scheduler to be set to either SCHED\textunderscore{}FIFO or
SCHED\textunderscore{}RR \cite{sched-setscheduler}. 
In order to provide a more intuitive interaction, a wrapper was written around
this system call. This provides a more 'D like' interface and a level of
abstraction. This wrapper takes the following form:
...
However, it is worth noting that on many operating systems, setting the scheduler 
...
an absolute time. In addition, this sleep must use a monotonic clock in order 
...
In D, the \texttt{Mutex} class follow a similar approach to the \texttt{Thread}
