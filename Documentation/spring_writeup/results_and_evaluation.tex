...
differently. In D, each \texttt{Thread} contains its own copy of any global 
variables, preventing them from being used globally, unless specifically stated 
...
\end{lstlisting}
%TC:endignore
D does not provide the notion of protected objects, but instead provides a
...
\end{lstlisting}
%TC:endignore
This allows the same principal, protected data, to be used in D for
safe interprocess communication using shared memory. However, the ability to 
define an \texttt{entry}, waiting on a particular condition to come true, is not 
encapsulated. However, it is possible to use the \texttt{Condition} class to
easily provide a simulated entry behaviour 
\cite{dlang-condition}. 
However, it is worth noting that using the above approach, it is not possible to 
use either the priority ceiling or priority inheritance protocol with the
\texttt{ReadWriteMutex}. To achieve this, the \texttt{ReadWriteMutex} class
must be redefined as using the corresponding \texttt{CeilingMutex} or
\texttt{InheritanceMutex}. 
