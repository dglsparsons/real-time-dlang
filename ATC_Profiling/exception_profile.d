...
void main()
{
    ...
    {
        totalSetupVariance += (time - (totalSetup/numtests))
                                .abs.total!"usecs".pow(2);
        totalTeardownVariance += (time - (totalSetup/numtests))
                                .abs.total!"usecs".pow(2);
    }
    writeln("Standard deviation of setup: ", 
                sqrt(totalSetupVariance /numtests));
    writeln("Standard deviation of teardown: ", 
                sqrt(totalTeardownVariance /numtests));
}
